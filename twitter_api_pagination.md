...
1. **Get Historical:** Requesting matching Posts from a time period of interest. These are typically one-time requests in support of historical research. Search requests can be based on `start_time` and `end_time` request parameters. The recent search endpoint responds with Posts delivered in reverse-chronological order, starting with the most recent matching Post.
2. **Polling:** Requesting matching Posts that have been posted since the last Post received. These use cases often have a near-real-time focus and are characterized by frequent requests, “listening” for new Posts of interest. The recent search endpoint provides the `since_id` request parameter in support of the ‘polling’ pattern. To help with navigating by Post IDs, the `until_id` request parameter is also available.
...
With polling use cases, “any new Posts of interest?” queries are made on an ongoing, frequent basis. Unlike historical use cases, that base requests on time, polling use cases typically base requests on Post IDs.
...
The recent search endpoint supports navigating the Post archive by Post ID. Responses from the endpoint include `oldest_id` and `newest_id` Post IDs. In the polling mode, requests are made with the `since_id` set to the largest/newest ID received so far.
...
When there is more data available, and `next_tokens` are provided, only the `newest_id` value from the first page of results is needed. Each page of data will include `newest_id` and `oldest_id` values, but the value provided in the first page is the only one needed for the next, regularly scheduled, polling request. 

Now say that there are now 18 more matching Posts. The endpoint would respond with this initial response with a full data page and a `next_token` for requesting the next page of data from this five-minute period. It would also include the newest Post ID needed for the next polling interval in five minutes:
```json
"meta": {
    "newest_id": "13800",
    "oldest_id": "12500",
